...
typedef struct PACKED _RobotCtrlState
{
	...
	float dribblerVel; // [rad/s] at motor, expected to be filtered with a time constant up to 10ms

	uint32_t ballIrState; // 0 = not visible, 1 = in range/detected, 2 = at dribbler (best state)
	float ballIrPos[2]; // [mm] in local "dribbler frame", zero is centered in front of dribbler
} RobotCtrlState;
...
typedef struct PACKED _RobotCtrlOutput
{
	...
	float dribblerVoltage;
	uint32_t dribblerMode;
} RobotCtrlOutput;

// output from trajectory generator
...
typedef struct PACKED _IRData
{
	LogEntryHeader header;
	float vLateral[4][5]; // Output of all scans of the IR-Sensor array [V]
	float estimatedBallPosition[2]; // X/Y [cm]
