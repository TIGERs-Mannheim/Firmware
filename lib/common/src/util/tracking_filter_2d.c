...
void TrackingFilter2DSetMeasError(TrackingFilter2D* pFilter, const float* pMeasError)
{
	MAT_ELEMENT(pFilter->kf.Ez, 0, 0) = pMeasError[0];
	MAT_ELEMENT(pFilter->kf.Ez, 1, 1) = pMeasError[1];
}

void TrackingFilter2DSetModelError(TrackingFilter2D* pFilter, float modelError)
{
	float dt = pFilter->dt;

	float dt3 = (1.0f / 3.0f) * dt * dt * dt * modelError;
	float dt2 = (1.0f / 2.0f) * dt * dt * modelError;
	float dt1 = dt * modelError;
