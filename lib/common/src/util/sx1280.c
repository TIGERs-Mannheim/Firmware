...
//#ifdef STM32F7XX
//static uint8_t tx[BUF_SIZE_RXTX] __attribute__((aligned(16), section(".dtc")));
//static uint8_t rx[BUF_SIZE_RXTX] __attribute__((aligned(16), section(".dtc")));
//#else
//static uint8_t tx[BUF_SIZE_RXTX] __attribute__((aligned(4)));
//static uint8_t rx[BUF_SIZE_RXTX] __attribute__((aligned(4)));
//#endif
...
static int16_t waitUntilReady(SX1280* pSX)
{
	...
	if(pSX->pBusyPort->IDR & pSX->busyPin)
	{
		if(chBSemWaitTimeout(&pSX->busyIrqSem, MS2ST(10)) != MSG_OK)
		{
...
static int16_t doTransfer(SX1280* pSX, uint32_t numBytes)
{
	...
	SPILowTransfer(&pSX->spi, pSX->pTx, pSX->pRx, numBytes);
...
static int16_t setAutoTx(SX1280* pSX, uint16_t timeUs)
{
...
{
	pSX->pTx = pData->pTxBuf;
	pSX->pRx = pData->pRxBuf;

	pSX->busyPin = pData->busyPin;
	pSX->pBusyPort = pData->pBusyPort;
	pSX->irqPin = pData->irqPin;
	pSX->pIrqPort = pData->pIrqPort;
	...
	GPIOInit(pData->pBusyPort, pData->busyPin, &gpioInit);

	gpioInit.extiTrigger = GPIO_EXTI_TRIG_RISING;
	gpioInit.pupd = GPIO_PUPD_DOWN;
	GPIOInit(pData->pIrqPort, pData->irqPin, &gpioInit);

	// init SPI
	pData->spiData.dmaPrio = 3;	// very high
	pData->spiData.cpha = 0;
	pData->spiData.cpol = 0;

	SPILowInit(&pSX->spi, &pData->spiData);
...
int16_t SX1280FastRxTxTransmit(SX1280* pSX, SX1280RxResult* pResult, uint8_t rxBytes, void* pRxData)
{
	...
	msg_t waitResult = chBSemWaitTimeout(&pSX->dataIrqSem, MS2ST(10));
