...
void RobotTask(void* params)
{
	...
	while(1)
	{
		...
		// run control algorithms for driving and dribbler
		if(robot.enabledSystems & ROBOT_SYSTEM_CONTROL)
			CtrlControl(&robot.state, &robot.skillOutput, &robot.ctrlOutput, &robot.ctrlRef);
...
static void sendMatchFeedback()
{
	...
	feedback.dribblerSpeed = fmaxf(0, robot.sensors.dribbler.speed * 1.0f/(2.0f*M_PI) *  60.0f * botParams.dribbler.motor2BarRatio * 0.01f);
	feedback.flags = (robot.sensors.ir.interrupted << 7) | (robot.sensors.kicker.kickCounter & 0x1F);
...
static void sendVersionInfo()
{
	...
	version.crc32 = BootGetApplicationCRC32();
