...
void FusionEKFUpdate(const RobotSensors* pSensors, const RobotCtrlOutput* pOutput, RobotCtrlState* pState)
{
	...
	pSlotNow->meas.gyrAcc[1] = pSensors->acc.linAcc[0];
	pSlotNow->meas.gyrAcc[2] = pSensors->acc.linAcc[1];
	...
	// DRIBBLER
	const float velLagT = 0.01f;

	++fusionEKF.dribbler.steadyHallCounter;

	uint8_t hallChanged = pSensors->dribbler.hallPos != fusionEKF.dribbler.lastHallPos;

	if(hallChanged || fusionEKF.dribbler.steadyHallCounter >= 10)
	{
		fusionEKF.dribbler.lastHallPos = pSensors->dribbler.hallPos;

		float dt = fusionEKF.dribbler.steadyHallCounter * 0.001f;
		fusionEKF.dribbler.steadyHallCounter = 0;

		if(hallChanged)
			fusionEKF.dribbler.filteredVelocity += dt/(velLagT+dt)*(pSensors->dribbler.speed - fusionEKF.dribbler.filteredVelocity);
		else
			fusionEKF.dribbler.filteredVelocity += dt/(velLagT+dt)*(0.0f - fusionEKF.dribbler.filteredVelocity);
	}

	pState->dribblerVel = fusionEKF.dribbler.filteredVelocity;
