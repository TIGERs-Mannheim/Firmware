...
void TestDriveDynamicResult(TestMotorDynamicsResult* pResult)
{
	...
	{
		snprintf(handles.mot[pResult->motorId].damp, 8, "N/A");
		snprintf(handles.mot[pResult->motorId].inertia, 8, "N/A");
	}
	else
	{
		snprintf(handles.mot[pResult->motorId].damp, 8, "%.0f", pResult->damping*1e6);
...
GHandle TestDriveCreate(TestDriveScheduleTestCallback scheduleTestCallback)
{
    ...
    GWidgetInit resLabelInit2 = { { 5, 71, 55, 16, TRUE, handles.hOverviewCont }, "164", 0, 0, 0 };
    gwinLabelCreate(0, &resLabelInit2);

    GWidgetInit emfLabelInit = { { 5, 95, 55, 16, TRUE, handles.hOverviewCont }, "J", 0, 0, 0 };
    gwinLabelCreate(0, &emfLabelInit);
    GWidgetInit emfLabelInit2 = { { 5, 111, 55, 16, TRUE, handles.hOverviewCont }, "4.5", 0, 0, 0 };
    gwinLabelCreate(0, &emfLabelInit2);

    GWidgetInit curLabelInit = { { 5, 135, 55, 16, TRUE, handles.hOverviewCont }, "i", 0, 0, 0 };
    gwinLabelCreate(0, &curLabelInit);
    GWidgetInit curLabelInit2 = { { 5, 151, 55, 16, TRUE, handles.hOverviewCont }, "0.06", 0, 0, 0 };
    gwinLabelCreate(0, &curLabelInit2);

    GWidgetInit velLabelInit = { { 5, 175, 55, 16, TRUE, handles.hOverviewCont }, "w", 0, 0, 0 };
    gwinLabelCreate(0, &velLabelInit);
    GWidgetInit velLabelInit2 = { { 5, 191, 55, 16, TRUE, handles.hOverviewCont }, "133", 0, 0, 0 };
    gwinLabelCreate(0, &velLabelInit2);
