...
This project contains code for TIGERs robots generation v2020 and for the base station v2018.
...
** libs: third-party libraries (non-TIGERs code), used by multiple applications
*** chibios: ChibiOS operating system files, used by main2019 and bs2018
...
** robot: *Primary robot control code* (control, sensor fusion, skills)
** shared: common code used by multiple applications
*** drv: drivers for specific (MCU-)external devices
*** gui: shared GUI elements
*** hal: Hardware Abstraction Layer (abstraction of MCU-internal components)
*** math: Algorithms e.g. for filters, control, trajectories
*** module: Larger software components build on top of drivers
*** net: UDP/IP network stack
*** protobuf-c: Generated protobuf message files
*** util: Utility functions and structures, not math-related
* cmake: CMake files
...
=== Get some help
The CLI has a built-in help system. Type `help` to list all available commands briefly.

Some commands accept arguments, you can use help to list them, e.g. `help led`.

Some commands are targeting a specific robot subsystem, e.g. `net`.
Subsystems require further arguments, you may list subsystem commands with `net help`. 
Note that help is now the second word, as it is forwarded to a subsystem. 
To get details of a subsystem command use e.g. `net help config`.

=== Add a new command
* Open the `shell_commands.c` file for your robot (src/main2019/shell_commands.c).
* Scroll down to the `ShellCommandsInit()` function
* Add your own command above this function with
+
[source,C,linenums]
----
SHELL_CMD(explode, "Make the robot go BOOM!");

SHELL_CMD_IMPL(explode)
{
...
----
+
* `SHELL_CMD` and `SHELL_CMD_IMPL` are macros doing some magic behind the scenes. The first
  describes your command (and potential arguments), the second defines the actual implementation.
* To make your command available you have to add it to the top-level shell command handler with.
  Go to the `ShellCommandsInit()` function and insert a line with
+
[source,C,linenums]
----
ShellCmdAdd(&devShell.shell.cmdHandler, explode_command);
----
+
* Note that `SHELL_CMD` automatically create a function with your command name extended with `_command`.

In your command handler you have just met another important function, `printf`.
It outputs data on the console interface. Note that you manually have to add carriage return ('\r') and new line ('\n').

Now select in the top menu Run => Run Configurations.. => Flash MB 2019 (Release) and click on Run.
...
Now enter `explode` into the console window and see what happens.

Note that your command was automatically added to `help` and `help explode` shows your command desription.
...
[source,C,linenums]
----
SHELL_CMD(pin, "Request PIN number from user",
	SHELL_ARG(number, "A very secret number")
);
...
SHELL_CMD_IMPL(pin)
{
	...
	printf("You entered PIN: %d\r\n", pin);
}
----

Remember to add the new `pin_command` to the `ShellCommandsInit()` function.

This command has one argument. Arguments are provided to the command implementation
via the `argc` (argument count) and `argv` (argument values) variables.

Note:
 * All argument variables are passed as strings!
 * The first argument is always the command name (hence, `argc` is always at least 1)
 * All arguments listed in `SHELL_CMD` are mandatory. If you do not supply enough arguments for a command you will receive an error message.
 * You may provide more arguments if the command is able to handle it, this is optional.
 * The shell command handler internally checks if all mandatory arguments are provided. Consequently, you rarely need to check the `argc` variable.
 * `pUser` is only intended for advanced use cases.

In the sample code, the argument that you provided is converted to an integer via `atoi` and then printed out. For floating point numbers you will need `atof`.

WARNING: `printf` requires a large thread stack and is extremely slow. Do not use it for high-frequency output.
...
Go to `src/robot` and have a look at `skill_sine.h` and `skill_sine.c` for a good and simple example.
...
Have a look at `skill_kick_ball.h` and `skill_kick_ball.c` in `src/robot`.
