...
== Shell
The Shell method is connected to the terminal as output. It offers printf functionality, which makes this function very versatile.

The call is `printf("format string", parameterrs..)`.
...
The terminal subsystem has limited buffering capabilities. If you output too much data at once with repeated printf calls the buffer
...
Only call printf on demand. Do not program periodic output via printf. This will clutter the terminal and make it hard to use (you no longer see what you type).

Technically, printf can be called from within any task which has a sufficiently large stack (4096B minimum, 8192B recommended).
...
=== Network
Analog to printf, there is a `NetworkPrint(..)` method. It works the same from a development perspective, also the same limits apply, and some more!
...
This method has an even lower bandwidth and buffering capability as printf. Furthermore, it can saturate the wireless link, leading to dropped packets. This can have severe impact on our game performance. Be very careful where you use this function.
...
TIP: SD Card logging may be unstable on v2020 robots if they are only powered via USB. Please use a battery or external power supply to provide enough power.
