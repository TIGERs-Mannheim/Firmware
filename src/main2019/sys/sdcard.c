...
#define SD_PIN_WP GPIO_PIN_12
#define SD_PIN_CD GPIO_PIN_11
#define SD_PIN_PWR GPIO_PIN_11

#define SD_PWR_ON() GPIOSet(GPIOD, SD_PIN_PWR)
#define SD_PWR_OFF() GPIOReset(GPIOD, SD_PIN_PWR)

#define SD_CLEAR_ALL_IRQ() (SDMMC1->ICR = 0x1FE00FFF)

SDCard sdCard;

static uint8_t dmaBuf[64];

CH_IRQ_HANDLER(Vector104) // SDMMC1
{
	CH_IRQ_PROLOGUE();
...
void SDCardInit()
{
	...
	// SD_PWR
	GPIOReset(GPIOD, GPIO_PIN_11); // pull low to disable SD card

	gpioInit.mode = GPIO_MODE_OUTPUT;
	gpioInit.otype = GPIO_OTYPE_PUSH_PULL;
	gpioInit.ospeed = GPIO_OSPEED_2MHZ;
	gpioInit.pupd = GPIO_PUPD_NONE;
	GPIOInit(GPIOD, GPIO_PIN_11, &gpioInit);

	// WP and CD
	gpioInit.mode = GPIO_MODE_INPUT;
	gpioInit.pupd = GPIO_PUPD_UP;
	GPIOInit(GPIOB, GPIO_PIN_12, &gpioInit);
	GPIOInit(GPIOF, GPIO_PIN_11, &gpioInit);
...
static uint32_t sendCmd(uint32_t cmd, uint32_t arg, uint32_t waitResponse)
{
	...
	SDMMC1->MASK = staMask;
	SDMMC1->ARG = arg;
	SDMMC1->CMD = cmd | (waitResponse << 8) | SDMMC_CMD_CPSMEN;
...
//	buffAddr &= 0xFFFFFFE0;	// align to 32 byte boundary
	...
	SDMMC1->IDMACTRL = 0;
	SDMMC1->ICR = SDMMC_ICR_IDMATEC | SDMMC_ICR_IDMABTCC;
	SDMMC1->IDMABASE0 = (uint32_t)pBuffer;
	SDMMC1->IDMACTRL |= SDMMC_IDMA_IDMAEN;

	SDMMC1->DTIMER = 25000000; // 1s
	SDMMC1->DLEN = numBytes;
	SDMMC1->DCTRL = SDMMC_DCTRL_DTEN | SDMMC_DCTRL_DTDIR | (dBlockSize << 4);
	SDMMC1->ARG = arg;
	SDMMC1->MASK = SDMMC_STA_CCRCFAIL | SDMMC_STA_CMDREND | SDMMC_STA_CTIMEOUT;
	SDMMC1->CMD = cmd | (SD_WAITRESP_SHORT << 8) | SDMMC_CMD_CPSMEN;
...
//	printf("Data done\r\nSTA: 0x%08X\r\n", SDMMC1->STA);
	...
	SCB_InvalidateDCache_by_Addr(pBuffer, numBytes);
...
//	printf("CMD done\r\nSTA: 0x%08X\r\n", sta);
	...
	SCB_CleanDCache_by_Addr((uint32_t*)pBuffer, numBytes);
	...
	SDMMC1->DTIMER = 25000000; // 1s
	SDMMC1->DLEN = numBytes;
	SDMMC1->DCTRL = SDMMC_DCTRL_DTEN | (dBlockSize << 4);
...
static int16_t initCard()
{
	...
	SDMMC1->CLKCR = 250;	// CLKDIV for 400kHz (during init)
...
//		printf("Voltage: %u, check: %02X\r\n", voltageAccepted, checkPattern);
	...
	while(chVTTimeElapsedSinceX(startTime) < TIME_MS2I(1000))
	{
		acmd41Runs++;
...
//			printf("ACMD41\r\nR3: 0x%08X\r\nTime: %u, STA: 0x%08X\r\n", R3, chVTGetSystemTimeX()-startTime, sta);
	...
	if(chVTTimeElapsedSinceX(startTime) >= TIME_MS2I(1000))
	{
	...
	{
		printf("CMD2: no response, 0x%08X\r\n", sta);
		return 1;
	}
...
//		printf("CMD3 finished.\r\nSTA: 0x%08X\r\nR6: 0x%08X\r\n", sta, R6);
	...
	{
		printf("CMD3: no response, 0x%08X\r\n", sta);
		return 1;
	}
	...
	{
		printf("CMD9: no response, 0x%08X\r\n", sta);
		return 1;
	}
...
//		printf("CMD13 finished.\r\nState: %u\r\nReady: %u\r\nApp: %u\r\n",
//				state, readyForData, appCmd);
	}
	else
	{
		printf("CMD13: no response, 0x%08X\r\n", sta);
		return 1;
	}
...
//				state, readyForData, appCmd);
	...
	if(sta & SDMMC_STA_CMDREND)
	{
//		printf("ACMD6 finished. Bus-Width: 4 bit\r\n");
	}
	else
	{
		printf("ACMD6: no response, 0x%08X\r\n", sta);
		return 1;
	}

	SDMMC1->CLKCR = 2 | SDMMC_CLKCR_WIDBUS_0 | SDMMC_CLKCR_HWFC_EN;	// CLKDIV for 25MHz
...
void SDCardTask(void* params)
{
	...
	while(1)
	{
		...
		if(sdCard.present == 0 && (GPIOF->IDR & SD_PIN_CD) == 0)
		{
			// card detected
			sdCard.writeProtected = ((GPIOB->IDR & SD_PIN_WP) == 0) ? 0 : 1;
		...
		if(sdCard.present == 1 && (GPIOF->IDR & SD_PIN_CD) != 0)
		{
