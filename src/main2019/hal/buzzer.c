...
#define BUZZER_ON() (TIM1->CCMR1 = TIM_CCMR1_OC1M_0 | TIM_CCMR1_OC1M_1 | TIM_CCMR1_OC1M_2)
#define BUZZER_OFF() (TIM1->CCMR1 = TIM_CCMR1_OC1M_0 | TIM_CCMR1_OC1M_2)
...
void BuzzerInit()
{
	...
	GPIOInit(GPIOA, GPIO_PIN_8, &gpioInit);

	TIM1->CR1 = TIM_CR1_ARPE;	// enable ARR preload
	TIM1->BDTR = TIM_BDTR_MOE;
	BUZZER_OFF();
	TIM1->CCER = TIM_CCER_CC1E | TIM_CCER_CC1P;	// enable CC1 output and set polarity to active low
	TIM1->PSC = 99;		// prescaler
	TIM1->ARR = 320;	// period
	TIM1->CCR1 = 103;	// pulse
	TIM1->EGR = TIM_EGR_UG;	// generate update event to take new ARR value
