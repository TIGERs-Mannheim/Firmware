...
#define CORTEX_PRIORITY_SVCALL 5
 ...
 */
#define CH_CFG_THREAD_INIT_HOOK(tp) { tp->p_ctime = 0; tp->p_cref = *DWT_CYCCNT; }
...
#define CH_CFG_CONTEXT_SWITCH_HOOK(ntp, otp) { \
	...
	if (otp) { \
		uint32_t cnt = ntp->p_cref - otp->p_cref; \
		otp->p_ctime += cnt; \
		cputime += cnt; \
	} \
	}
