...
uint32_t chThdGetFreeStack(void* pThread)
{
	...
	uint8_t* pEnd = (uint8_t*)pThd->p_ctx.r13;
	uint8_t* pStack = (uint8_t*)(pThd+1);

	if(&ch.mainthread == pThread)
	{
		pEnd = (uint8_t*)&__process_stack_end__;
		pStack = (uint8_t*)&__process_stack_base__;
		pThd = (thread_t*)&__process_stack_base__;
	}

	if(pThread == 0)
	{
		pEnd = (uint8_t*)&__main_stack_end__;
		pStack = (uint8_t*)&__main_stack_base__;
		pThd = (thread_t*)&__main_stack_base__;
	}

	while(pStack < pEnd && *pStack == CH_DBG_STACK_FILL_VALUE)
		pStack++;

	uint32_t stackFree = (uint32_t)pStack - (uint32_t)pThd;
