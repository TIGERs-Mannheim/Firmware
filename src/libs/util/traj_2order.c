...
void TrajSecOrder2DAsyncCreate(TrajSecOrder2DAsync* pTraj, const float* pX0, const float* pXd0, const float* pXTrg, float xdMax, float xddMax, float rotation)
{
	...
	CtrlUtilRotate(-rotation, startToTarget[0], startToTarget[1], startToTarget);

	float xd0Rotated[2];
	CtrlUtilRotate(-rotation, pXd0[0], pXd0[1], xd0Rotated);
...
void TrajSecOrder2DAsyncValuesAtTime(const TrajSecOrder2DAsync* pTraj, float t, float* pX, float* pXd, float* pXdd)
{
	...
	if(pX)
	{
		CtrlUtilRotate(pTraj->rotation, x.x, y.x, pX);
	...
	if(pXd)
	{
		CtrlUtilRotate(pTraj->rotation, x.xd, y.xd, pXd);
	}

	if(pXdd)
	{
		CtrlUtilRotate(pTraj->rotation, x.xdd, y.xdd, pXdd);
	}
}
