...
void LogFileOpen(const char* pName)
{
	...
	if(pName)
	{
		...
		if(chMBPost(&logFile.eventQueue, EVENT_OPEN_NAMED, MS2ST(500)) != MSG_OK)
		{
	...
	{
		if(chMBPost(&logFile.eventQueue, EVENT_OPEN_AUTO, MS2ST(500)) != MSG_OK)
		{
			LogError("post failed");
		}
	}
...
void LogFileClose()
{
	if(chMBPost(&logFile.eventQueue, EVENT_CLOSE, MS2ST(500)) != MSG_OK)
	{
		LogError("post failed");
	}
}
...
void LogFileTask(void* params)
{
	...
	while(1)
	{
		if(chVTTimeElapsedSinceX(logFile.lastStatsTime) >= MS2ST(1000))
		{
		...
		if((uint32_t)logFile.pBufWrite > (uint32_t)logFile.pBufStart+bufSize/2)
			writeData();
		...
		if(chMBFetch(&logFile.eventQueue, &event, MS2ST(1)) == MSG_OK)
		{
			switch(event)
			{
				case EVENT_OPEN_AUTO:
				{
					...
					ConsolePrint("Opening logfile with auto naming\r\n");
				...
				case EVENT_OPEN_NAMED:
				{
					...
					ConsolePrint("Opening named logfile: %s\r\n", logFile.logFilename);
