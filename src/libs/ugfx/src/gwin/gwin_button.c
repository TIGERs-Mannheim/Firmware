...
static const GColorSet *getButtonColors(GWidgetObject *gw) {
	...
	void gwinButtonDraw_Normal(GWidgetObject *gw, void *param) {
		...
		gdispGDrawLine(gw->g.display, gw->g.x, gw->g.y, gw->g.x, gw->g.y+gw->g.height-1, pcol->edge);
		gdispGDrawLine(gw->g.display, gw->g.x, gw->g.y, gw->g.x+gw->g.width-2, gw->g.y, pcol->edge);
