...
static int16_t botTransfer(uint8_t isInTransfer, uint8_t* pData, uint32_t dataLength, uint32_t* pBytesRead)
{
	...
	if(result)
	{
		ConsolePrint("Transfer CBW failed: 0x%04X\r\n", result);
		return result;
	}
	...
	if(result)
	{
		ConsolePrint("Transfer data failed: 0x%04X\r\n", result);
		return result;
	}
	...
	if(result)
	{
		ConsolePrint("Transfer CSW failed: 0x%04X\r\n", result);
		return result;
	}
...
static void init(USBDeviceDesc* pDeviceDesc, uint8_t* pConfigDesc, uint16_t cfgDescSize)
{
	...
	{
		ConsolePrint("Unsupported command set\r\n");
		return;
	}
	...
	{
		ConsolePrint("Unsupported protocol\r\n");
	}
	...
	{
		ConsolePrint("Missing bulk endpoint\r\n");
		return;
	}
	...
	if(usbMsc.pEPIn == 0)
	{
		ConsolePrint("Creating IN endpoint failed\r\n");
		return;
	}
	...
	if(usbMsc.pEPOut == 0)
	{
		ConsolePrint("Creating OUT endpoint failed\r\n");
		return;
	}

	ConsolePrint("Issuing MSC reset...");
	...
	if(result)
		ConsolePrint("GetMaxLUN failed\r\n");
	else
		ConsolePrint("Max LUNs: %u\r\n", usbMsc.numLogicalUnits);
...
//			(uint16_t)requestSense.ASC, (uint16_t)requestSense.ASCQ);
	...
	if(result)
	{
		ConsolePrint("ReadCapacity failed: 0x%04X\r\n", result);
		return;
	}

	ConsolePrint("Disk capacity: %uMB\r\nBlock Size: %u\r\n", usbMsc.numBlocks/((1024*1024)/usbMsc.blockSize), usbMsc.blockSize);
...
static void stop()
{
	...
	if(fresult != FR_OK)
	{
		ConsolePrint("FatFS umount error: %u\r\n", (uint32_t)fresult);
		return;
	}
