...
//		ConsolePrint("USB Host mode\r\n");
//	else
//		ConsolePrint("USB Client mode\r\n");
...
int16_t USBIRPSubmit(USBEndpoint* pEP, USBIRP* pIRP)
{
	...
	chMBPost(&usb.eventQueue, EVENT_BEGIN_IRP | (pEP->chNum << 8), TIME_INFINITE);
...
void USBTask(void* params)
{
	...
	while(1)
	{
		if(chMBFetch(&usb.eventQueue, &event, TIME_INFINITE) != MSG_OK)
			continue;
...
//				USB_OTG_FS_HOST->HFIR = 48000;
			...
			ConsolePrint("Port enabled\r\n");

			switch(speed)
			{
				case 1: ConsolePrint("Full Speed\r\n"); break;
				case 2: ConsolePrint("Low Speed\r\n"); break;
				default: ConsolePrint("Unkown Speed\r\n"); break;
			}
		...
		if(event & EVENT_PORT_DISABLED)
		{
			...
			for(uint8_t ep = 0; ep < USB_MAX_ENDPOINTS; ep++)
			{
				...
				for(USBIRP* pIRP = pEP->pIRPList; pIRP; pIRP = pIRP->pNext)
				{
					ConsolePrint("-> %hu", (uint16_t)pIRP->state);
				}
			}
...
/*
		...
		{
			ConsolePrint("Port overcurrent\r\n");
		}

		if(event & EVENT_DISCONNECT)
		{
			ConsolePrint("Device disconnected\r\n");
...
//				ConsolePrint("HCCHAR: 0x%08X\r\n", pEP->pChannel->HCCHAR);

//				pEP->pChannel->HCCHAR |= USB_OTG_HCCHAR_CHDIS;
				chThdSleepMilliseconds(10);

//				ConsolePrint("CH %hu flushed\r\n", (uint16_t)ep);
...
//	ConsolePrint("RX. CH: %u, Bytes: %u, pktsts: %02X\r\n", ch, bytes, pktsts);
	...
	if(pIRP == 0)
	{
		...
		ConsolePrint("No IRP pending for CH %u!\r\n", ch);
	}
	else
	{
...
static void eventChIRQ(uint32_t ch)
{
	...
	if(chMBFetch(&pEP->eventQueue, &chEvent, TIME_IMMEDIATE) != MSG_OK)
		return;
	...
	{
		ConsolePrint("EP%hu data toggle error\r\n", (uint16_t)pEP->chNum);
	...
	{
		ConsolePrint("EP%hu frame overrun\r\n", (uint16_t)pEP->chNum);
	}

	if(chEvent & USB_OTG_HCINT_BBERR)
	{
		ConsolePrint("EP%hu babble error\r\n", (uint16_t)pEP->chNum);
