...
static void testKickerUpdate(Test* pTest, void* pUser);
...
static int16_t eventHandler(GEvent* pEvent)
{
	if(pEvent->type == GEVENT_GWIN_BUTTON)
	{
		...
		{
			TestScheduleWithUpdates(TEST_ID_KICKER, 0, 1, &testKickerUpdate, 0);
...
static void testKickerUpdate(Test* pTest, void* pUser)
{
	...
	{
		const TestResultKicker* pResult = (const TestResultKicker*)TestGetResult(pTest);
	...
	else
	{
		...
		gwinSetText(&handles.hProgress->w.g, buf, FALSE);
	}
}

GHandle TestKickerCreate()
{
