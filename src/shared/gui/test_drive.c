...
static void testDriveUpdate(Test* pTest, void* pUser);
	...
	if(pEvent->type == GEVENT_GWIN_BUTTON)
	{
		...
		if(pBtn->gwin == handles.hBtnTestAll)
		{
			for(uint8_t i = 0; i < 5; i++)
			{
				...
				TestScheduleWithUpdates(TEST_ID_MOT_IDENT_MECH, &arg, 1, &testDriveUpdate, 0);
			}
...
static void testDriveUpdate(Test* pTest, void* pUser)
{
	...
	{
		const TestResultMotIdentMech* pResult = (const TestResultMotIdentMech*)TestGetResult(pTest);
	...
	else
	{
		...
		gwinSetText(&handles.mot[pArg->motorId].hDamping->w.g, buf, FALSE);
	}
}

GHandle TestDriveCreate()
{
