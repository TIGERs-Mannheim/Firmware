...
static void changeModeXY(TrajGenerator* pGen, uint8_t newMode)
{
	...
	switch(pGen->modeXY)
	{
		case MODE_RESET:
		{
			...
			Vector2fTurnGlobal2Local(pGen->filterGlobalPos[2], pGen->filterGlobalVel[0], pGen->filterGlobalVel[1], pGen->trajLocalVel, pGen->trajLocalVel+1);
		...
		case MODE_GLOBAL_POS:
		{
			switch(newMode)
			{
				case MODE_LOCAL_VEL:
				{
					Vector2fTurnGlobal2Local(pGen->trajGlobalPos[2], pGen->trajGlobalVel[0], pGen->trajGlobalVel[1], pGen->trajLocalVel, pGen->trajLocalVel+1);
					Vector2fTurnGlobal2Local(pGen->trajGlobalPos[2], pGen->trajGlobalAcc[0], pGen->trajGlobalAcc[1], pGen->trajLocalAcc, pGen->trajLocalAcc+1);
				}
				break;
			}
		}
		...
		case MODE_LOCAL_VEL:
		{
			switch(newMode)
			{
				...
				case MODE_GLOBAL_POS:
				{
					...
					Vector2fTurnLocal2Global(pGen->filterGlobalPos[2], pGen->trajLocalVel[0], pGen->trajLocalVel[1], pGen->trajGlobalVel, pGen->trajGlobalVel+1);
					Vector2fTurnLocal2Global(pGen->filterGlobalPos[2], pGen->trajLocalAcc[0], pGen->trajLocalAcc[1], pGen->trajGlobalAcc, pGen->trajGlobalAcc+1);
				}
				break;
			}
		}
...
void TrajGeneratorStepXY(TrajGenerator* pGen, float tStep)
{
	switch(pGen->modeXY)
	{
		...
		case MODE_LOCAL_VEL:
		{
			...
			Vector2fTurnLocal2Global(pGen->trajGlobalPos[2], posDiff[0], posDiff[1], posDiff, posDiff+1);
