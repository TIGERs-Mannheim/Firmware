...
#define CRCt(crc, ch)	 (crc = (crc >> 8) ^ crctab[(crc ^ (ch)) & 0xff])
...
uint32_t CRC32CalcChecksum(const void* pData, uint32_t dataLength)
{
	...
	return CRC32StopChecksum(crc);
}


uint32_t CRC32VerifyChecksum(const void* pData, uint32_t dataLength, uint32_t crc)
{
	uint32_t crcv = CRC32StartChecksum();

	crcv = CRC32FeedChecksum(crcv, pData, dataLength);
	crcv = CRC32FeedChecksum(crcv, &crc, sizeof(uint32_t));

	return CRC32StopChecksum(crcv);
}

/*
 * CRC-16/XMODEM
 * Calculate CRC16 (xmodem) using table-driven method
 * width=16 poly=0x1021 init=0x0000 refin=false refout=false xorout=0x0000 check=0x31c3 residue=0x0000
 * name="CRC-16/XMODEM"
 * referring to http://reveng.sourceforge.net/crc-catalogue/16.htm
 */
uint16_t CRC16FeedChecksum(uint16_t crc, const void* _pData, uint32_t dataLength)
{
    ...
    while(dataLength--)
        crc = crc16tab[((crc >> 8) ^ *pData++)] ^ (crc << 8);
...
uint16_t CRC16CalcChecksum(const void* _pData, uint32_t dataLength)
{
    ...
    return CRC16FeedChecksum(crc, _pData, dataLength);
}
