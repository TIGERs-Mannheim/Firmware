...
ConfigFile* ConfigOpenOrCreate(const ConfigFileDesc* pConfigDesc, void* pData, uint16_t size, ConfigUpdateCallback updateCb, uint8_t flags)
{
	...
	if((flags & CONFIG_FILE_FLAG_VOLATILE) == 0)
	{
...
{
	if((pFile->flags & CONFIG_FILE_FLAG_VOLATILE) == 0)
...
{
	chMBPostTimeout(&config.eventQueue, event, TIME_IMMEDIATE);
}
...
void ConfigTask(void* params)
{
	...
	while(1)
	{
		msg_t result = chMBFetchTimeout(&config.eventQueue, &event, TIME_MS2I(100));
		...
		if(chVTTimeElapsedSinceX(lastCfg2LogTime) > TIME_MS2I(1000))
		{
		...
		switch(internalEvent)
		{
			case CONFIG_EVENT_SEND_FILE_LIST:
			{
				for(uint16_t i = 0; i < config.filesUsed; i++)
				{
					...
					if(pFile->flags & CONFIG_FILE_FLAG_INTERNAL)
						continue;
