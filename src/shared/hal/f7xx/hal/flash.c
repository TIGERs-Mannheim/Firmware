...
int16_t FlashErase(uint32_t addr, uint32_t endAddr)
{
	...
	uint32_t numSectors = sizeof(sectorStartAddresses)/sizeof(sectorStartAddresses[0]);

	uint32_t startSector = numSectors-1;
	for(uint32_t i = 0; i < numSectors-1; i++)
	{
		if(addr < sectorStartAddresses[i+1])
		{
			startSector = i;
			break;
		}
	...
	for(uint32_t i = numSectors-1; i > 0; i--)
	{
		if(endAddr > sectorStartAddresses[i])
		{
			endSector = i;
			break;
		}
