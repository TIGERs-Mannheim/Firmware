...
void UartDmaInit(UartDma* pUart, UartDmaData* pInit, tprio_t prio)
{
	...
	pUart->pDmaRx->CR = DMA_DIR_PERIPH2MEM | (1 << 20) | pInit->dmaPrio | DMA_SxCR_MINC | DMA_SxCR_CIRC;
	pUart->pDmaRx->FCR = 0;
	pUart->pDmaRx->M0AR = (uint32_t)pInit->pDmaBufRx;
	pUart->pDmaRx->PAR = (uint32_t)&pRegister->RDR;
	pUart->pDmaRx->NDTR = pInit->dmaBufSizeRx;

	pUart->pDmaTx->CR = DMA_DIR_MEM2PERIPH | (1 << 20) | pInit->dmaPrio | DMA_SxCR_MINC | DMA_SxCR_TCIE;
	pUart->pDmaTx->FCR = 0;
	...
	while(1)
	{
		...
		if((events & (EVENT_MASK_BREAK_DETECTED | EVENT_MASK_IDLE_LINE)) || events == 0)
		{
			...
			if(bytesEmpty < pUart->rxMinEmptySize)
			{
				...
				if(pUart->pRxRead + bytesAdvance < pEnd)
				{
			...
			chDbgAssert(pUart->pRxRead >= pBegin && pUart->pRxRead < pEnd, "pRxRead out of bounds");
