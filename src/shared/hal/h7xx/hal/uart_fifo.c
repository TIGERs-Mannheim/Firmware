...
void UARTFifoIRQ(UARTFifo* pUART)
{
	...
	if(sr & (USART_ISR_RXFT | USART_ISR_IDLE | USART_ISR_PE | USART_ISR_FE | USART_ISR_NE | USART_ISR_ORE))
	{
		...
		while(pUART->pRegister->ISR & USART_ISR_RXNE_RXFNE)
		{
	...
	if(pUART->pTxData)
	{
		...
		if(sr & USART_ISR_TC)
		{
			...
			else
			{
				...
				if(pUART->sendBreakAfterTx)
					pUART->pRegister->RQR = USART_RQR_SBKRQ;
		...
		{
			// fill it up!
			while(pUART->pRegister->ISR & USART_ISR_TXE_TXFNF)
			{
...
void UARTFifoInit(UARTFifo* pUART, USART_TypeDef* pRegister, uint32_t baudrate, uint32_t perClk, uint8_t enableParity)
{
	...
	pRegister->CR2 = 0;
	pRegister->CR3 = USART_CR3_RXFTCFG_0 | USART_CR3_TXFTCFG_1;
