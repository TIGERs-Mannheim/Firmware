...
{
	if(connect)
		chMBPostTimeout(&usbHcd.eventQueue, EVENT_CONNECT, TIME_MS2I(200));
	else
		chMBPostTimeout(&usbHcd.eventQueue, EVENT_DISCONNECT, TIME_MS2I(200));
}
...
int16_t USBGetString(uint8_t index, char* pDest, uint16_t dstLen)
{
	...
	if(result)
	{
		printf("Reading string failed: 0x%04X\r\n", result);
		return result;
	}
...
{
	chMBPostTimeout(&usbHcd.eventQueue, event, TIME_INFINITE);
}
...
void USBHCDTask(void* params)
{
	...
	while(1)
	{
		if(chMBFetchTimeout(&usbHcd.eventQueue, &event, TIME_MS2I(10)) != MSG_OK)
		{
		...
		if(event & EVENT_CONNECT)
		{
			...
			if(result)
			{
				printf("Reading initial device desc failed: 0x%04X\r\n", result);
				continue;
			}
			...
			{
				printf("Max packet size: %hu, reconfiguring endpoints...\r\n", (uint16_t)pDevDesc->bMaxPacketSize);
			...
			if(result)
			{
				printf("Set address failed: 0x%04X\r\n", result);
				continue;
			}
			...
			if(result)
			{
				printf("Reading full device desc failed: 0x%04X\r\n", result);
				continue;
			}
			...
			if(result)
			{
				printf("Reading first language code failed: 0x%04X\r\n", result);
				continue;
			}

			usbHcd.langCode = *((uint16_t*)(rxBuf+2));
			printf("First language code: %04hX\r\n", usbHcd.langCode);
			...
			if(result)
			{
				printf("Reading configuration descriptor failed: 0x%04X\r\n", result);
				continue;
			}

			printf("Config descriptor length: %hu", configLen);
			...
			for(uint8_t config = 0; config < usbHcd.deviceDesc.bNumConfigurations; config++)
			{
				...
				for(uint8_t interface = 0; interface < pConfigDesc->bNumInterfaces; interface++)
				{
					...
					for(uint8_t endpoint = 0; endpoint < pInterfaceDesc->bNumEndpoints; endpoint++)
					{
						...
						{
							case 0: printf("      Type:       Control\r\n"); break;
							case 1: printf("      Type:       Isochronous\r\n"); break;
							case 2: printf("      Type:       Bulk\r\n"); break;
							default: printf("      Type:       Interrupt\r\n"); break;
						}
						printf("      maxPktSize: %hu\r\n", pEPDesc->wMaxPacketSize);
			...
			{
				printf("No proper class found for this device\r\n");
				continue;
			}

			printf("Loading class driver: %s\r\n", usbHcd.pActiveClass->pName);
