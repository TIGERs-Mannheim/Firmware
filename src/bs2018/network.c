	...
	while (1)
	{
		...
		if(chVTTimeElapsedSinceX(lastStatsTime) > MS2ST(100))
		{
			...
			for(uint8_t i = 0; i < WIFI_MAX_BOTS; i++)
			{
				...
				network.bsWifi.bots[slotsUsed].bsRssi = (int16_t)(fmaxf(wifi.bots[i].emaRssi[0].value, wifi.bots[i].emaRssi[1].value)*10.0f);
				network.bsWifi.bots[slotsUsed].botRssi = (int16_t)(wifi.bots[i].botRssi.value*10.0f);
