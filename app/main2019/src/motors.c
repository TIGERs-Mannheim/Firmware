...
#define ENCODER_DELTA_TO_RAD_PER_SEC_DRIBBLER (2.181661564992912f)
#define RAD_PER_SEC_TO_ENCODER_DELTA_DRIVE (3.666929888837269f)
#define RAD_PER_SEC_TO_ENCODER_DELTA_DRIBBLER (0.4583662361046586f)
...
static const ConfigFileDesc configFileDescMotorsFoc =
	{ SID_CFG_MOTORS_FOC, 0, "motors/foc", 8, (ElementDesc[]) {
...
static const ConfigFileDesc configFileDescMotors =
	{ SID_CFG_MOTORS_VEL, 5, "motors/vel", 2, (ElementDesc[]) {
...
{
	motors.focConfig.dribblerDKp = FixedPointUnsignedSaturate(kp*GAIN_REAL_TO_MOTOR_CTRL_DQ, 7, 10)*GAIN_MOTOR_TO_REAL_CTRL_DQ;
	motors.focConfig.dribblerDKi = FixedPointUnsignedSaturate(ki*GAIN_REAL_TO_MOTOR_CTRL_DQ, 5, 13)*GAIN_MOTOR_TO_REAL_CTRL_DQ;
...
{
	motors.focConfig.dribblerQKp = FixedPointUnsignedSaturate(kp*GAIN_REAL_TO_MOTOR_CTRL_DQ, 7, 10)*GAIN_MOTOR_TO_REAL_CTRL_DQ;
	motors.focConfig.dribblerQKi = FixedPointUnsignedSaturate(ki*GAIN_REAL_TO_MOTOR_CTRL_DQ, 5, 13)*GAIN_MOTOR_TO_REAL_CTRL_DQ;
...
{
	motors.velConfig.driveKp = FixedPointUnsignedSaturate(kp*GAIN_REAL_TO_MOTOR_CTRL_VEL, 7, 10)*GAIN_MOTOR_TO_REAL_CTRL_DQ;
	motors.velConfig.driveKi = FixedPointUnsignedSaturate(ki*GAIN_REAL_TO_MOTOR_CTRL_VEL, 5, 13)*GAIN_MOTOR_TO_REAL_CTRL_DQ;
...
static void sendMotorData(uint32_t motorId)
{
	...
	if(motorId > 3)
	{
		pMosi->curCtrlDKp = FixedPointSignedFromFloat(motors.focConfig.dribblerDKp*GAIN_REAL_TO_MOTOR_CTRL_DQ, 10);
		pMosi->curCtrlDKi = FixedPointSignedFromFloat(motors.focConfig.dribblerDKi*GAIN_REAL_TO_MOTOR_CTRL_DQ, 13);
		pMosi->curCtrlQKp = FixedPointSignedFromFloat(motors.focConfig.dribblerQKp*GAIN_REAL_TO_MOTOR_CTRL_DQ, 10);
		pMosi->curCtrlQKi = FixedPointSignedFromFloat(motors.focConfig.dribblerQKi*GAIN_REAL_TO_MOTOR_CTRL_DQ, 13);
		pMosi->velCtrlKp = 0;
		pMosi->velCtrlKi = 0;
	}
	else
	...
	switch(pMotor->mode)
	{
		...
		case MOTOR_MODE_SPEED:
		{
			...
			if(motorId > 3)
				pMosi->encDeltaSetpoint = (int16_t)(RAD_PER_SEC_TO_ENCODER_DELTA_DRIBBLER * pMotor->setVelocity);
			else
