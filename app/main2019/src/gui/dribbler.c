	...
	if(pEvent->type == GEVENT_GWIN_BUTTON)
	{
		...
		for(uint8_t i = 0; i < 4; i++)
		{
			if(pBtn->gwin == handles.hBtnPresets[i])
			{
				...
				RobotTestModeSetSystems(ROBOT_SYSTEM_DRIBBLER | ROBOT_SYSTEM_STATE_EST | ROBOT_SYSTEM_CONTROL);

				robot.skillOutput.dribbler.mode = presets[i].mode;
				robot.skillOutput.dribbler.speed = presets[i].speed;

				ctrlPanthera.pConfigDribbler->iZero = presets[i].current;
				ctrlPanthera.pConfigDribbler->deltaCurrent = 0.0f;
				ctrlPanthera.pConfigDribbler->deltaSpeed = 1.0f;
				ctrlPanthera.pConfigDribbler->fwdAccCur = 0.0f;
				ctrlPanthera.pConfigDribbler->fwdBrkCur = 0.0f;
				ctrlPanthera.pConfigDribbler->revAccCur = 0.0f;
				ctrlPanthera.pConfigDribbler->revBrkCur = 0.0f;
			}
		}
	}
